...
<template>
  <div
    class="relative flex items-start gap-2 p-4 rounded backdrop-blur border-white/10 bg-white/20"
  >
    ...
    <IconExclamationCircle
      v-if="isError"
      class="w-5 h-5 text-[#E26868] shrink-0"
    />
    <IconExclamation
      v-if="isWarning"
    ...
    <div class="text-sm">
      ...
      <div class="leading-tight text-white capitalize-first">
        {{ notification.text }}
      </div>
      <div
        v-if="slots.footer"
        class="text-[10px] italic leading-tight text-gray-400 mt-2"
      >
