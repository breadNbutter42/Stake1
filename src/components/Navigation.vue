...
<template>
  <div
    class="sticky top-0 z-10 h-16 px-6 backdrop-blur border-b border-white/10"
  >
    <div class="flex items-center justify-between h-full">
      <div class="flex items-center justify-end flex-1 gap-4">
        <div v-if="isAuthenticated">
          ...
          <div
            class="flex items-center justify-end gap-2 text-xs text-white text-right whitespace-nowrap"
          >
            {{ chainName }}
          </div>
        </div>
            ...
            wallet
              ? isAppNetwork
                ? isAuthenticated
                  ...
                  : "Connect Wallet"
                : "Wrong Network"
              : "Unable to detect Avalanche provider"
          }}
        </Button>
      </div>
    </div>
